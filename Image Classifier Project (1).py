...
def testClassfier(model,criterion,testloader,current_device):
    model.to(current_device)
    test_loss=0
    accuracy=0
...
def trainClassifier(model,epochs_number,criterion,optimizer,training_loader,validation_loader,current_device):
    model.to(current_device)
...
epochs_number=5
learning_rate=0.001
optimizer=optim.Adam(model.classfier.parameters(),lr=learning_rate)

with active_session():
    train_loss,valid_loss,valid_accuracy=trainClassfier(model,epochs_number,criterion,optimizer,training_loader,validation_loader,current_device)
    
print("result:","Training loss: {train_loss:.3f}..\n",
      "Testing loss: {valid_loss:.3f}.. \n",
      "Testing accuracy: {valid_accuracy:.3f}")
filename =saveCheckpoint(model)
...
with active_session():
    train_loss, valid_loss, valid_accuracy = trainClassifier(
        model, epochs_no, criterion, optimizer, training_loader, 
        validation_loader, current_device)

print("Result:",
      "Train loss: {train_loss:.3f}.. \n",
      "Test loss: {valid_loss:.3f}.. \n",
      "Test accuracy: {valid_accuracy:.3f}")
...
def convertCatToName(categories, mapper='cat_to_name.json'):
...
names = convertCatToName(classes)
print(probs)
print(classes)
print(names)
...
def select_rnd_image(path):
    rnd_folder = random.choice(os.listdir(path))
    rnd_file = random.choice(
    os.listdir(os.path.join(path, rnd_folder)))
    image_path = os.path.join(path, rnd_folder, rnd_file)
    return image_path
...
def display_preds(path, model, topk=5, flwr_name=None):
    image_path = select_rnd_image(path)
    fldr_no = image_path.split('/')[2]
    title = flwr_name[fldr_no]
...
for n in range(no_of_predictions):
    display_preds('flowers/test', model_from_file, flwr_name=cat_to_name)
